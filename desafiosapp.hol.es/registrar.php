			...
			/*if(isset($_POST['email']) && !empty($_POST['email']) && esEmail($_POST['email'])) {*/
				if(isset($_POST['password']) && !empty($_POST['password']) && strlen($_POST['password']) > 6) {
					/*if(isset($_POST['password2']) && $_POST['password'] == $_POST['password2']) {*/
						...
						$usuario = asegurar(trim(strtolower($_POST['usuario'])));
						...
						if($consultaUsuarios->num_rows == 0) {
							...
							if($consultaEmails->num_rows == 0) {*/
								...
								$con->query("INSERT INTO usuarios (USUARIO, EMAIL, CONTRASENA, ESTADO) VALUES ('$usuario', '', '$nuevaClave', 'activo')");/*pendiente*/
						...
						} else { /*http_response_code(400); */echo "Ya existe una cuenta con ese usuario."; }
					/*} else { http_response_code(400); echo "4"; }*/
				} else { http_response_code(400); echo "La contraseña debe tener mas de 6 caracteres."; }
