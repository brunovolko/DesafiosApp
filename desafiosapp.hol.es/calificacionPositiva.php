	...
	if(isset($_POST["token"]) && esSeguro($_POST["token"]) && isset($_POST['idPublicacion']) && is_numeric($_POST['idPublicacion']) && esSeguro($_POST['idPublicacion'])) {
		...
		if($consulta->num_rows == 1) {
			...
			if($consulta->num_rows == 1) {
				...
				//Ahora hay que ver si ya lo habia calificado positivamente
				$consulta = $con->query("SELECT IDCALIFICACION FROM calificaciones WHERE IDUSUARIO='$idusuario' AND IDPUBLICACION='$idPublicacion' AND CALIFICACION='1' LIMIT 1");

				if($consulta->num_rows == 1) {
					echo "Ya lo habia calificado";
					// Ya lo habia calificado positivamente, por lo tanto, hay que sacar esa calificacion
					...
					//No lo habia calificado, lo califica y por las dudas borro un posible negativo
					$con->query("INSERT INTO calificaciones (IDPUBLICACION, IDUSUARIO, CALIFICACION) VALUES ('$idPublicacion', '$idusuario', '1')");
