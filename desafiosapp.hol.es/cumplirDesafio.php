	...
	if(isset($_POST["token"]) && esSeguro($_POST["token"]) && isset($_POST['desafio']) && is_numeric($_POST['desafio']) && isset($_FILES['imagen']) && $_FILES["imagen"]["type"] == "image/jpg") {
		...
		if($consulta->num_rows == 1) {
			...
			if($consultaDesafioOk->num_rows == 1) {
				...
				$consultaYaLoCumpli = $con->query("SELECT IDPUBLICACION FROM publicaciones WHERE IDDESAFIO='$idDesafio' AND IDUSUARIO='$idusuario' LIMIT 1");
				if($consultaYaLoCumpli->num_rows == 0) {
					...
					if($con->query($query)) {
						...
						$ultimaPublicacion = $consultaUltimaPublicacion->fetch_array()["IDPUBLICACION"];
